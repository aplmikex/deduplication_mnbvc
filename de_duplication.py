...
def check(file_json, windows_size=5):
    ...
    if file_json['hash_length']/(len(file_json['content'])) < 0.9:
        ...
        for i in range(len(file_json['content'])-windows_size):
            hashs_sum_each_windows_size.append(sum([file_json['content'][j]['hash'] for j in range(i, i+windows_size)]))
        ...
        if len(hashs_sum_each_windows_size) > 0 and len(sets)/len(hashs_sum_each_windows_size) < 0.9:
            for tmpset in sets:
                if hashs_sum_each_windows_size.count(tmpset) > 1:
                    indices = [i for i, x in enumerate(hashs_sum_each_windows_size) if x == tmpset][1:]
...
def run_process(file_path_queue):
...
def remove_duplicates(src_dir, n_process=4):
    assert os.path.exists(src_dir)
    file_path_queue = get_all_files(src_dir, legal_file_type=['.jsonl'])

    processes = []
    for _ in range(n_process):
        p = Process(target = run_process, args = (file_path_queue, ))
        p.start()
        processes.append(p)
    for p in processes:
        p.join()
...
if __name__ == '__main__':
    remove_duplicates('converted/', n_process=multiprocessing.cpu_count())